...
export const FilesApp: React.FC = () => {
  ...
  return (
    <div
      id="files-app"
      className={`flex h-full w-full select-none relative transition-colors ${
        isLightMode ? 'bg-slate-50 text-slate-900' : 'bg-[#0c0c10] text-[#f4f4f5]'
      }`}
      onDragOver={(e) => {
        e.preventDefault();
    ...
    >
      ...
      <div
        className={`w-52 border-r flex flex-col p-3 gap-1 transition-colors ${
          isLightMode ? 'bg-slate-100/90 border-slate-200 text-slate-800' : 'border-[#27272a]/60 bg-[#111116] text-zinc-300'
        }`}
      >
        <div className="px-2 py-2 mb-1">
          <span className={`text-[10px] font-bold uppercase tracking-wider ${isLightMode ? 'text-slate-500' : 'text-zinc-400'}`}>
            Speicherorte
          </span>
        </div>
        ...
        <div
          className={`mt-auto p-2.5 rounded-xl border text-[11px] space-y-1 transition-colors ${
            isLightMode ? 'bg-white border-slate-200 text-slate-700' : 'bg-[#171720] border-[#27272a]/60 text-zinc-300'
          }`}
        >
          <div className="flex items-center gap-1.5 font-semibold" style={{ color: accentConfig.primary }}>
            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />
            <span>Cloud-Tresor</span>
          </div>
          <p className={`text-[10px] ${isLightMode ? 'text-slate-500' : 'text-zinc-400'}`}>
            Alle Objekte sind E2E mit AES-GCM gesichert.
          </p>
        </div>
      </div>

      {/* Main Files Area */}
      <div className={`flex-1 flex flex-col overflow-hidden transition-colors ${isLightMode ? 'bg-slate-50' : 'bg-[#0c0c10]'}`}>
        {/* Top Control Bar */}
        <div
          className={`h-12 border-b px-4 flex items-center justify-between transition-colors ${
            isLightMode ? 'bg-white border-slate-200' : 'bg-[#121217] border-[#27272a]/60'
          }`}
        >
          {/* Breadcrumbs */}
          <div className="flex items-center gap-1 text-xs overflow-x-auto max-w-md">
            {currentFolderId && (
              <button
                onClick={() => {
                ...
                className={`p-1 rounded mr-1 transition-colors ${
                  isLightMode ? 'hover:bg-slate-100 text-slate-700' : 'hover:bg-[#22222e] text-zinc-300'
                }`}
                title="Ebene höher"
              >
            ...
            {breadcrumbs.map((crumb, idx) => (
              <React.Fragment key={crumb.id || 'root'}>
                {idx > 0 && <ChevronRight className={`w-3 h-3 shrink-0 ${isLightMode ? 'text-slate-400' : 'text-zinc-600'}`} />}
                <button
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              <Search className={`w-3.5 h-3.5 absolute left-2.5 top-1/2 -translate-y-1/2 ${isLightMode ? 'text-slate-400' : 'text-zinc-400'}`} />
              <input
                type="text"
                placeholder="Dateien filtern..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className={`w-36 sm:w-48 pl-8 pr-2.5 py-1.5 rounded-xl text-xs border focus:outline-none transition-colors ${
                  isLightMode
                    ? 'bg-slate-100 border-slate-300 text-slate-900 placeholder-slate-400 focus:bg-white focus:border-purple-500'
                    : 'bg-[#1a1a24] border-[#27272a] text-white placeholder-zinc-500 focus:border-purple-500'
                }`}
              />
            </div>
            ...
            <button
              ...
              className={`p-1.5 rounded-lg border transition-all ${
                isLightMode
                  ...
                  : 'text-zinc-300 hover:text-white hover:bg-[#22222e] border-[#27272a]'
              }`}
              title="Neuer Ordner"
            >
            ...
            <div
              className={`flex border rounded-lg overflow-hidden ${
                isLightMode ? 'border-slate-200 bg-slate-100' : 'border-[#27272a] bg-[#181822]'
              }`}
            >
              <button
                ...
                style={viewMode === 'grid' ? { backgroundColor: accentConfig.primary } : undefined}
              >
              ...
              <button
                ...
                style={viewMode === 'list' ? { backgroundColor: accentConfig.primary } : undefined}
              >
        ...
        {(isCreatingFolder || isCreatingFile) && (
          <div
            className={`p-3 border-b flex items-center gap-3 ${
              isLightMode ? 'bg-slate-100 border-slate-200' : 'bg-[#171722] border-[#27272a]'
            }`}
          >
            ...
            <input
              ...
              className={`px-3 py-1.5 rounded-lg border text-xs focus:outline-none ${
                isLightMode
                  ...
                  : 'bg-[#0e0e14] border-purple-500 text-white'
              }`}
            />
            <button
              onClick={() => {
              ...
              className="px-3 py-1.5 rounded-lg text-xs font-bold text-white bg-purple-600 shadow-sm"
            ...
            <button
              ...
              className={`text-xs ${isLightMode ? 'text-slate-500 hover:text-slate-900' : 'text-zinc-400 hover:text-white'}`}
            >
              Abbrechen
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {currentItems.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-8">
              <Folder className={`w-12 h-12 mb-2 ${isLightMode ? 'text-slate-300' : 'text-zinc-700'}`} />
              <p className={`text-sm font-semibold ${isLightMode ? 'text-slate-700' : 'text-zinc-400'}`}>
                Dieser Ordner ist leer
              </p>
              <p className={`text-xs mt-1 ${isLightMode ? 'text-slate-500' : 'text-zinc-600'}`}>
                Ziehe Dateien hierher oder erstelle neue Dokumente.
              </p>
            </div>
          ) : viewMode === 'grid' ? (
            /* Grid View with FLIP layout */
                ...
                return (
                  <motion.div
                    ...
                    className={`group relative p-3 rounded-2xl border flex flex-col items-center text-center cursor-pointer transition-all ${
                      isSelected
                        ? isLightMode
                          ? 'bg-purple-50/80 border-purple-400 ring-1 ring-purple-300 shadow-md'
                  ...
                  >
                    ...
                    >
                      ...
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" title="Verschlüsselt synchronisiert" />
                    </div>
                    ...
                    <div className="absolute top-1.5 right-1.5 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      {item.type !== 'folder' && (
                        <button
                          ...
                          className={`p-1 rounded-md transition-colors ${
                            isLightMode
                              ? 'bg-slate-100 text-slate-700 hover:text-white hover:bg-purple-600'
                              : 'bg-[#22222d] text-zinc-300 hover:text-white hover:bg-purple-600'
                          }`}
                          title="Herunterladen"
                        >
                      ...
                      {!item.isProtected && (
                        <button
                          ...
                          className={`p-1 rounded-md transition-colors ${
                            isLightMode
                              ? 'bg-slate-100 text-slate-700 hover:text-red-600 hover:bg-red-50'
                              : 'bg-[#22222d] text-zinc-300 hover:text-red-400 hover:bg-red-950'
                          }`}
                          title="Löschen"
                        >
            ...
            /* List View with FLIP layout */
            <div
              className={`border rounded-2xl overflow-hidden transition-colors ${
                isLightMode ? 'bg-white border-slate-200' : 'bg-[#111116] border-[#22222a]'
              }`}
            >
              <table className="w-full text-left text-xs">
                <thead
                  className={`border-b text-[11px] ${
                    isLightMode ? 'bg-slate-100/80 border-slate-200 text-slate-600' : 'bg-[#171720] border-[#22222a] text-zinc-400'
                  }`}
                >
                  <tr>
                ...
                <tbody className={`divide-y ${isLightMode ? 'divide-slate-200' : 'divide-[#22222a]'}`}>
                  {currentItems.map((item) => (
                    <motion.tr
                      layout
                      key={item.id}
                      ...
                      className={`cursor-pointer transition-colors ${
                        isLightMode ? 'hover:bg-slate-50' : 'hover:bg-[#181822]'
                      }`}
                    >
                      <td className="py-2 px-4 flex items-center gap-2.5 font-medium">
                        {item.type === 'folder' ? (
                          <Folder className="w-4 h-4 text-purple-500" />
                        ) : (
                          <FileText className={`w-4 h-4 ${isLightMode ? 'text-slate-500' : 'text-zinc-400'}`} />
                        )}
                        <span className={`truncate max-w-xs ${isLightMode ? 'text-slate-800' : 'text-zinc-200'}`}>{item.name}</span>
                      </td>
                      <td className={`py-2 px-4 capitalize ${isLightMode ? 'text-slate-600' : 'text-zinc-400'}`}>{item.type}</td>
                      <td className={`py-2 px-4 font-mono ${isLightMode ? 'text-slate-600' : 'text-zinc-400'}`}>
                        {item.type === 'folder' ? '—' : `${item.size} B`}
                      </td>
                      <td className={`py-2 px-4 font-mono text-[11px] ${isLightMode ? 'text-slate-500' : 'text-zinc-500'}`}>
                      ...
                      <td className="py-2 px-4 text-right space-x-1">
                        {item.type !== 'folder' && (
                          <button
                            ...
                            className={`p-1 rounded ${isLightMode ? 'text-slate-600 hover:text-slate-900' : 'text-zinc-400 hover:text-white'}`}
                          >
                        ...
                        {!item.isProtected && (
                          <button
                            ...
                            className={`p-1 rounded ${isLightMode ? 'text-slate-600 hover:text-red-600' : 'text-zinc-400 hover:text-red-400'}`}
                          >
      ...
      <AnimatePresence>
        {editingFile && (
          <motion.div
            ...
            className={`absolute inset-y-0 right-0 w-full md:w-3/5 border-l shadow-2xl flex flex-col z-30 ${
              isLightMode ? 'bg-white border-slate-300 text-slate-900' : 'bg-[#121218] border-[#27272a] text-[#f4f4f5]'
            }`}
          >
            {/* Editor Header */}
            <div
              className={`h-12 border-b px-4 flex items-center justify-between ${
                isLightMode ? 'bg-slate-100 border-slate-200' : 'bg-[#15151f] border-[#27272a]'
              }`}
            >
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  className={`bg-transparent font-semibold text-xs border-b border-transparent focus:outline-none ${
                    isLightMode
                      ? 'text-slate-900 hover:border-slate-400 focus:border-purple-500'
                      : 'text-white hover:border-zinc-500 focus:border-purple-500'
                  }`}
                />
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className={`p-1 rounded-lg ${
                    isLightMode ? 'text-slate-500 hover:text-slate-900 hover:bg-slate-200' : 'text-zinc-400 hover:text-white hover:bg-[#22222e]'
                  }`}
                >
                  <X className="w-4 h-4" />
                </button>
            ...
            {/* Editor Body */}
            <div className={`flex-1 p-4 overflow-y-auto ${isLightMode ? 'bg-slate-50' : 'bg-[#0a0a0e]'}`}>
              {editingFile.type === 'image' ? (
                <div className="h-full flex items-center justify-center p-4">
                  {editingFile.content.startsWith('<svg') ? (
                    <div
                      className={`max-w-xs max-h-xs p-4 rounded-2xl border ${
                        isLightMode ? 'bg-white border-slate-200 shadow-md' : 'bg-[#14141c] border-zinc-800'
                      }`}
                ...
                <textarea
                  ...
                  className={`w-full h-full bg-transparent font-mono text-xs resize-none focus:outline-none leading-relaxed ${
            ...
            <div
              className={`h-8 border-t px-4 flex items-center justify-between text-[10px] font-mono ${
                isLightMode ? 'bg-slate-100 border-slate-200 text-slate-600' : 'bg-[#121217] border-[#27272a] text-zinc-500'
              }`}
            >
              <span>
                {editorContent.length} Zeichen • {editorContent.split(/\s+/).filter(Boolean).length} Wörter
              </span>
              <span className="text-emerald-500 font-medium">AES-GCM verschlüsselter Speicher</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
